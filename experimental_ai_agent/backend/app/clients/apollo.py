...
class ApolloClient:
    ...
    async def search_people(self, company: SimpleCompany, personas: Optional[List[PersonaConfig]] = None, max_leads: int = 5) -> List[Dict[str, Any]]:
        ...
        if company.id:
            payload["organization_ids"] = [company.id]
            print(f"[Apollo People] Searching with organization_ids filter: {company.id}")
        else:
            print(f"[Apollo People] WARNING: No organization_id provided for {company.name} - may return random leads")
